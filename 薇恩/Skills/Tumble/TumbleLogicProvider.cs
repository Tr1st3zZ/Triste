...
namespace TDVayne.Skills.Tumble
{
    internal class TumbleLogicProvider
    {
        ...
        public Vector3 GetTDVayneQPosition()
        {
            ...
            var enemiesNear =
                EntityManager.Heroes.Enemies.Where(m => m.IsValidTarget(m.GetAutoAttackRange() + 300f + 65f)).ToList();
            ...
            var closeNonMeleeEnemy =
                TumbleHelper.GetClosestEnemy(ObjectManager.Player.ServerPosition.Extend(Game.CursorPos, 300f).To3D());
            ...
            if (ObjectManager.Player.HealthPercent < 10 && ObjectManager.Player.CountEnemiesInRange(1500) > 1)
            {
                ...
                return position.IsSafeEx() ? (Vector3) position : endPosition;
            }
        ...
        {
            if (!Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Combo) || !Variables.E.IsEnabledAndReady())
            {
                return Vector3.Zero;
            }

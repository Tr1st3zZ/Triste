...
namespace TDVayne.Skills.Tumble
{
    class TumbleLogicProvider
    {
        ...
        public Vector3 GetTDVayneQPosition()
        {
            ...
            var enemiesNear =
                HeroManager.Enemies.Where(m => m.IsValidTarget(m.GetAutoAttackRange() + 300f + 65f)).ToList();
            #endregion
        ...
        {
            if (!Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Combo) || !Variables.E.IsReady())
            {
                return Vector3.Zero;
            }

    ...
    class Program : Helper
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            if (RecallNoob)
            {
                ...
                {
                    Drawing.DrawText(300, 235, Color.White, "寻找回城点");
                }
                else
                {
                    Drawing.DrawText(300, 235, Color.White, "回城中~");
                } 
            }
            if (ComboPLS)
            {
                Drawing.DrawText(300, 220, Color.White, "连招开启");
            }
            if (Waitingminions)
            {
                Drawing.DrawText(300, 205, Color.White, "等待小兵...");
            }
            if (ChangedToAllies)
            {
                Drawing.DrawText(300, 190, Color.White, "跟随队友");
            }
            if (LeaveTowerPls)
            {
                Drawing.DrawCircle(tower.Position, 1000, Color.Red);
                Drawing.DrawText(157, 800, Color.Red, "离开塔下");
            }
